...
function applyDiff(originalContent, diffOutput) {
  ...
  if (!originalContent.includes(findText)) {
    ...
    const replLines  = replaceText.split("\n");

    // Strategy 1: normalized contiguous block match
    // Try matching find lines contiguously after whitespace normalization
    let startIdx = -1;
    outer: for (let i = 0; i <= origLines.length - findLines.length; i++) {
      for (let j = 0; j < findLines.length; j++) {
        if (normalize(origLines[i + j]) !== findLines[j]) continue outer;
      }
      startIdx = i;
      break;
    }
    ...
    if (startIdx !== -1) {
      return [
        ...
        ...replLines,
        ...origLines.slice(startIdx + findLines.length)
      ].join("\n");
    }

    // Strategy 2: single key line match
    // If FIND is multi-line but only one line is actually the value being changed,
    // find the most specific line (longest, most unique) and replace just that line.
    // This handles cases like DeepSeek wrapping a single CSS property in its selector block.
    const keyLine = findLines.reduce((a, b) => a.length >= b.length ? a : b, "");
    const replKeyLine = replLines.reduce((a, b) => a.length >= b.length ? a : b, "");

    const keyIdx = origLines.findIndex(l => normalize(l) === keyLine);
    if (keyIdx !== -1 && keyLine.length > 5) {
      const origIndent = origLines[keyIdx].match(/^\s*/)[0];
      origLines[keyIdx] = origIndent + replKeyLine.trim();
...
app.post("/api/suggest", requireAuth, async (req, res) => {
  try {
    ...
    const absPath = path.resolve(PROJECT_ROOT, filePath);
    console.log(`[suggest] PROJECT_ROOT: ${PROJECT_ROOT}`);
    console.log(`[suggest] Resolved path: ${absPath}`);

    if (!fs.existsSync(absPath)) {
      return res.status(404).json({ error: `File not found: ${filePath} (resolved to ${absPath})` });
    }
