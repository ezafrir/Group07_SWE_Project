...
// ============================================================
// NEW ITERATION ROUTE: 3 LLM COMPARISON
// ============================================================
app.post("/api/compare", requireAuth, async (req, res) => {
  ...
  try {
    // 1. Define the 3 different LLMs you have available in Ollama
    const modelsToCompare = ["llama3.2", "mistral", "gemma:2b"]; 
    
    const responsePromises = modelsToCompare.map((model) => 
      generateLLMResponse(prompt.trim(), model)
    );
    
    const rawResponses = await Promise.all(responsePromises);

    // Map them together for the frontend
    const mappedResponses = rawResponses.map((text, index) => ({
      model: modelsToCompare[index],
      text: text
    }));

    // 2. Generate the comparison summary
    const comparisonPrompt = `Compare these 3 AI responses to the prompt "${prompt}". 
    List the key similarities and differences in a concise format.\n\n` + 
    mappedResponses.map((r) => `[${r.model} Response]:\n${r.text}`).join("\n\n");

    // Use your main/strongest model to judge the comparison
    const comparisonSummary = await generateLLMResponse(
      comparisonPrompt, 
      "llama3.2", 
      "You are a critical comparison judge."
    );
    ...
    res.status(502).json({ error: `Comparison failed: ${err.message}` });
  }
});
...
app.delete("/api/bookmarks/:id", requireAuth, (req, res) => {
  const id = Number(req.params.id);
  const conversation = unbookmarkConversation(id, req.session.user.id);
  if (!conversation) {
    return res.status(404).json({ error: "Conversation not found." });
  }
